...
const Login = ({ onLogin }) => {
  ...
  // Check for messages from previous navigation (e.g., successful verification)
  React.useEffect(() => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          ...
          if (errorData.requires_verification) {
            setError(errorData.error || "Please verify your email address first.");
            setTimeout(() => {
              navigate('/signup', { 
                state: { 
      ...
      if (data.status === "success") {
        ...
        // Use the secure login handler passed from App
        if (onLogin) {
