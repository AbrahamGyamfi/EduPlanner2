...
def register_behavioral_analytics_routes(app):
    ...
    @app.route('/behavioral-analytics/<user_id>/ai-enhanced', methods=['GET'])
    def get_ai_enhanced_behavioral_analytics(user_id):
        """Get AI-enhanced behavioral analytics with intelligent insights"""
        try:
            # Validate user exists
            try:
                user_object_id = ObjectId(user_id) if isinstance(user_id, str) else user_id
            ...
            if use_ai:
                try:
                    ...
                    ai_insights = generate_ai_insights(analytics, quiz_stats, subject_performance, user_id)
                    response_data['ai_insights'] = ai_insights
                    response_data['ai_enhanced'] = True
                except ImportError:
                    logger.warning("AI Analytics module not available")
            ...
            logger.error(f"Error retrieving AI-enhanced behavioral analytics: {str(e)}")
            return jsonify({'error': 'Error retrieving AI-enhanced behavioral analytics'}), 500
